...
export const AddTransaction = () => {
  ...
  return (
    <>
      <h3>Plan New Income/Expense</h3>
      <form onSubmit={onSubmit}>
        <div className="form-control">
          <label htmlFor="text">Description</label>
          <input type="text" value={text} onChange={(e) => setText(e.target.value)} placeholder="e.g. Monthly Rent" required />
        </div>
        <div className="form-control">
        ...
        <div className="form-control">
          <label htmlFor="date">Start Date / Due Date</label>
          <input type="date" value={date} onChange={(e) => setDate(e.target.value)} required />
        </div>
        <div className="form-control">
          <label htmlFor="note">Notes</label>
          <textarea value={note} onChange={(e) => setNote(e.target.value)} placeholder="Extra details..." />
        </div>
        <button className="btn">Set Recurring Plan</button>
      </form>
    </>
  );
};
